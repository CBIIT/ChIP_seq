...
required options:
  ...
  -t  <string>  Type (Cufflink expression: 'EXP' , Cuffdiff: 'DIFF', Matrix file: 'MATRIX')
  -b  <string>  Input BED file  

optional options:  
  -g  <integer> column index of gene symbol (start with 0. default: $gene_col)  
  -m  <integer> column index of value(FPKM/TPM) (start with 0. default: $value_col)  
  -d  <string>  TAD file
...
if (!$exp_type || ($exp_type ne 'EXP' && $exp_type ne 'DIFF' && $exp_type ne 'MATRIX')) {
...
sub readMatrix {
  ...
  while (<FILE>) {
	chomp;
	my @fields = split(/\t/);
	my $g = $fields[$gene_col];
	my $gene_id = $g;
	if ($g =~ /(.*?)_/) {
		$gene_id = $1;	
	}
	my $fpkm = $fields[$value_col];
	next if ($fpkm < $fpkm_cutoff);
